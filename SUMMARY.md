...
看板根据最近 3 小时的加权负面占比超过配置阈值，或最近一小时热度超过历史均值指定倍数来提示风险。看板同时支持下载明细、小时指标和预测结果。预警是规则型提示，不替代人工研判。

## 5. 本次工程优化

- NLP 使用领域词典最长匹配，减少中文单字碎片和重叠 n-gram。
- 情感评分增加简单否定词处理，并校验输入字段。
- 采集增加参数校验、URL 规范化和异常互动量容错。
- 小时指标自动补齐空小时，避免把没有数据误读为趋势下降。
- 配置采用默认值深合并，局部 YAML 配置不会丢失其他默认项。
